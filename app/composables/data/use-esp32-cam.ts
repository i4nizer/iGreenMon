...
export const useEsp32Cam = () => {
	...
	const create = async (
		...
		try {
			const res = await $fetch<Esp32Cam>(
				"/api/user/greenhouse/esp32-cam",
				{
					method: "POST",
					body: esp32Cam,
				}
	...
	const retrieve = async (id: number): Promise<SafeResult<Esp32Cam>> => {
		try {
	...
	const destroy = async (id: number): Promise<SafeResult<Esp32Cam>> => {
		try {
			const url = `/api/user/greenhouse/esp32-cam/${id}`
	...
	const retrieveKey = async (id: number): Promise<SafeResult<string>> => {
		try {
			const reqFetch = useRequestFetch()
			const url = `/api/user/greenhouse/esp32-cam/key?esp32camid=${id}`
