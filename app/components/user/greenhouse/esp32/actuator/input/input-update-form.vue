<template>
	<VeeForm
		...
		@submit="onSubmit"
	>
		<h2 class="text-green">Update Input</h2>
...
// --- Validation
const validationSchema = toTypedSchema(InputUpdateSchema)

// --- Data Binding
const emit = defineEmits<{
	error: [msg: string]
	submit: [input: InputUpdate]
	success: [input: Input]
}>()
...
const props = defineProps<{
	pins: Pin[]
	input: Input
	handler?: (values: InputUpdate, event: any) => any
}>()

// --- CRUD
const { update } = useInput()

const onSubmit = async (values: any, event: any) => {
	const input = values as InputUpdate
    emit("submit", input)
